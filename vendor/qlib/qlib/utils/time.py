...
CRYPTO_TIME = [datetime.strptime("0:00", "%H:%M"), datetime.strptime("23:59", "%H:%M")]
...
def get_min_cal(shift: int = 0, region: str = REG_CN) -> List[time]:
    ...
    if region == REG_CN:
        ...
        for ts in list(
            pd.date_range(CRYPTO_TIME[0], CRYPTO_TIME[1], freq="1min") - pd.Timedelta(minutes=shift)
        ):
            cal.append(ts.time())
    else:
...
def time_to_day_index(time_obj: Union[str, datetime], region: str = REG_CN):
    ...
    if region == REG_CN:
        ...
        raise ValueError(f"{time_obj} is not within the opening time of the {region} market")
    else:
        raise ValueError(f"{region} is not supported")
