"""Strategy templates - predefined trading strategy configurations."""
...
# Type definitions for template fields
...
STRATEGY_TEMPLATES: list[StrategyTemplate] = [
    # Momentum Strategies
    {
        "id": "momentum_basic",
        "name": "Basic Momentum",
        "description": "Simple price momentum strategy using 20-day and 60-day returns. "
    ...
    },
    {
        ...
        "description": "Advanced momentum strategy with volume confirmation and volatility adjustment. "
    ...
    # Mean Reversion Strategies
    {
        ...
        "description": "Classic mean reversion strategy buying oversold assets and selling overbought ones. "
    ...
    },
    {
        ...
        "description": "Pairs trading and statistical mean reversion using cointegration and z-score signals. "
    ...
    # Multi-Factor Strategies
    {
        ...
        "description": "Balanced multi-factor approach combining quality, value, and momentum factors. "
    ...
    },
    {
        ...
        "description": "Risk parity allocation across multiple factors. "
    ...
    # Crypto-Specific Strategies
    {
        ...
        "description": "Trend-following strategy optimized for cryptocurrency markets. "
    ...
    },
    {
        ...
        "description": "Short-term mean reversion for crypto markets. "
    ...
    },
    {
        ...
        "description": "Yield-focused strategy for DeFi protocols. "
...
def _validate_template(template: StrategyTemplate) -> list[str]:
    """Validate a single template and return list of errors."""
    errors: list[str] = []
    ...
    # Validate factors not empty
    factors = template.get("factors", [])
    if not factors:
        errors.append(f"Template '{template_id}': factors list is empty")

    # Validate factor_descriptions match factors
    factor_descriptions = template.get("factor_descriptions", {})
    if set(factor_descriptions.keys()) != set(factors):
        missing = set(factors) - set(factor_descriptions.keys())
        extra = set(factor_descriptions.keys()) - set(factors)
        if missing:
            errors.append(
                f"Template '{template_id}': missing descriptions for {missing}"
            )
        if extra:
            errors.append(f"Template '{template_id}': extra descriptions for {extra}")

    # Validate numeric bounds
    max_drawdown = template.get("max_drawdown", 0)
    if not (0 <= max_drawdown <= 1):
        errors.append(f"Template '{template_id}': max_drawdown must be between 0 and 1")

    position_size_limit = template.get("position_size_limit", 0)
    if not (0 <= position_size_limit <= 1):
        errors.append(
            f"Template '{template_id}': position_size_limit must be between 0 and 1"
        )

    max_positions = template.get("max_positions", 0)
    if max_positions < 1:
        errors.append(f"Template '{template_id}': max_positions must be >= 1")

    return errors


def validate_all_templates() -> None:
    """Validate all strategy templates at startup.

    Raises:
        ValueError: If any template validation fails.
    """
    all_errors: list[str] = []

    # Check for duplicate IDs
    ids = [t.get("id") for t in STRATEGY_TEMPLATES]
    duplicates = [id_ for id_ in ids if ids.count(id_) > 1]
    if duplicates:
        all_errors.append(f"Duplicate template IDs: {set(duplicates)}")
    ...
    logger.info(
        "All %d strategy templates validated successfully", len(STRATEGY_TEMPLATES)
    )


# Validate templates at module import time
